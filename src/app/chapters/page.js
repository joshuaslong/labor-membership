    ...
    const loadData = async () => {
      ...
      if (user) {
        const { data: teamMember } = await supabase
          ...
          .eq('active', true)
          .single()
        setIsAdmin(teamMember?.roles?.some(r => ['super_admin', 'national_admin', 'state_admin', 'county_admin', 'city_admin'].includes(r)))
      }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl text-gray-900">Chapters</h1>
          <p className="text-gray-600">Browse the chapter hierarchy</p>
        </div>
        {isAdmin && (
          <Link href="/admin/chapters/new" className="btn-primary w-full sm:w-auto text-center">
            Create Chapter
          </Link>
        )}
      </div>
