...
export default function WorkspaceProfilePage() {
  ...
  if (!member) {
    return <div className="max-w-xl mx-auto px-4 py-12 text-center text-sm text-gray-500">Profile not found</div>
  }

  return (
    <div className="max-w-xl mx-auto px-4 sm:px-6 py-6">
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-gray-900 tracking-tight">My Profile</h1>
        <p className="text-sm text-gray-500 mt-1">Update your contact information and preferences.</p>
      </div>

      {error && <div className="bg-red-50 text-red-700 p-4 rounded-lg mb-4 text-sm">{error}</div>}
      {success && <div className="bg-green-50 text-green-700 p-4 rounded-lg mb-4 text-sm">Profile updated!</div>}
